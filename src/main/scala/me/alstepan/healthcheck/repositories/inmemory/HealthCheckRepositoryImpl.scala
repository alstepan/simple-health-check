...
class HealthCheckRepositoryImpl[F[_]: Concurrent](repo: Ref[F, Seq[HealthCheckResult]]) extends HealthCheckRepository[F] {
  override def saveCheckResults(results: Seq[HealthCheckResult]): F[Unit] =
    repo.update(l => (l ++ results).takeRight(1000))

  override def getResults(services: Set[ServiceId], start: Timestamp, end: Timestamp): F[Seq[HealthCheckResult]] =
    repo.get.map(l =>
      l.filter(r => r.time.after(start) && r.time.before(end) && (services.isEmpty || services.contains(r.id) ))
    )

  override def getFailures(services: Set[ServiceId], start: Timestamp, end: Timestamp): F[Seq[HealthCheckResult]] =
...
object HealthCheckRepositoryImpl {
  def apply[F[_]: Concurrent]: F[HealthCheckRepository[F]] =
    for {
      results <- Ref.of[F, Seq[HealthCheckResult]](List())
